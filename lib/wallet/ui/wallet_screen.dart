...
class _WalletScreenState extends State<WalletScreen> {
  ...
  void _openRazorpayCheckout({
    ...
    final options = {
      ...
      'prefill': {
        ...
        // Provide fallback email for Razorpay test mode. Replace for production deployments.
        'email': 'test@example.com',
      },
      'theme': {
        'color':
            '#FFD200', // TODO: replace with production palette if required.
      },
    };
  ...
  Future<void> _debitWallet({
    ...
    if (result.ok) {
      _showSnack('₹$amountInRupees debited successfully.');
      await _loadBalance(silently: true);
    } else {
      _showSnack(result.message);
    }
